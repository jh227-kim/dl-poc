...
AUTO_OFFSET = os.environ.get("KAFKA_AUTO_OFFSET_RESET", "earliest")
...
def kafka_loop() -> None:
    ...
    while True:
        try:
            print(f"[{SERVICE_LABEL}] Kafka 연결 시도 중... (bootstrap={KAFKA_BOOTSTRAP})")
            ...
            print(f"[{SERVICE_LABEL}] Kafka 연결 성공 및 구독 중: topic={KAFKA_READY_TOPIC} group={CONSUMER_GROUP}")
            break
        except Exception as e:
            print(f"[{SERVICE_LABEL}] Kafka 연결 실패 ({e}). {retry_interval}초 후 재시도...")
    ...
    try:
        ...
        print(f"[{SERVICE_LABEL}] Kafka 루프 중 오류 발생: {e}")
