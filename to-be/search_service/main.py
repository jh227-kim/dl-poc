...
SERVICE_LABEL = "검색서비스"
CONSUMER_GROUP = os.environ.get("KAFKA_CONSUMER_GROUP", "search-to-be")
KAFKA_BOOTSTRAP = os.environ.get("KAFKA_BOOTSTRAP_SERVERS", "localhost:9092")
KAFKA_INGEST_TOPIC = os.environ.get("KAFKA_INGEST_TOPIC", "mail.events")
SUPPLY_SERVICE_URL = os.environ.get("SUPPLY_SERVICE_URL", "http://localhost:8100")
...
def handle_event(mail_id: str | None, action: str | None) -> None:
    log = f"[{SERVICE_LABEL}] Kafka 이벤트 수신 | mail_id={mail_id}, action={action}"
    print(log)
    logs.append(log)

    log2 = f"[{SERVICE_LABEL}] 공급서비스에 직접 원본 데이터 요청 → GET {SUPPLY_SERVICE_URL}/mails/{mail_id}"
    print(log2)
    logs.append(log2)
...
def kafka_loop() -> None:
    ...
    print(f"[{SERVICE_LABEL}] 구독 중: topic={KAFKA_INGEST_TOPIC} group={CONSUMER_GROUP}")
    for message in consumer:
        body = message.value
        if not isinstance(body, dict):
            continue
        handle_event(body.get("mail_id"), body.get("action"))
