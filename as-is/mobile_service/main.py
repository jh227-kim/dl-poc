...
app = FastAPI(title="모바일서비스 - As-Is")
...
def callback(message):
    ...
    log = f"[모바일서비스] 이벤트 수신 | mail_id={mail_id}, action={action}"
    print(log)
    logs.append(log)

    # ★ As-Is 핵심 문제: 공급서비스에 직접 원본 데이터 요청
    log2 = f"[모바일서비스] 공급서비스에 직접 원본 데이터 요청 → GET {SUPPLY_SERVICE_URL}/mails/{mail_id}"
    print(log2)
    logs.append(log2)
    ...
    try:
        ...
        log3 = f"[모바일서비스] 데이터 수신 완료 | mail_id={mail_id} | 응답시간: {data.get('response_delay_sec', 0):.1f}초"
        print(log3)
        logs.append(log3)
    except Exception as e:
        logs.append(f"[모바일서비스] 오류: {e}")
...
def start_subscriber():
    ...
    print(f"[모바일서비스] {SUBSCRIPTION} 구독 시작")
...
@app.get("/")
def health():
    return {"service": "모바일서비스 (As-Is)", "status": "running", "received_count": len(received_data)}
